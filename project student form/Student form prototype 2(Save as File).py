...
def backup():
    with open("student1.csv",mode="w") as myfile:
        riter=csv.writer(myfile,lineterminator="\n")
        count=UID-10800
        qb=["UID","Name","Age","Gender","D.O.B.","Father's Name","Mother's Name","Email","Phone Number","House Address","Year of Admission","Course"]
        riter.writerow(qb)
        for i in range(count):
            qb=[]
            qb.append(studentlist[i].UID)
            qb.append(studentlist[i].name)
            qb.append(studentlist[i].age)
            qb.append(studentlist[i].gender)
            qb.append(studentlist[i].dob)
            qb.append(studentlist[i].fathername)
            qb.append(studentlist[i].mothername)
            qb.append(studentlist[i].email)
            qb.append(studentlist[i].number)
            qb.append(studentlist[i].address)
            qb.append(studentlist[i].adyear)
            qb.append(studentlist[i].course)
            riter.writerow(qb)
    return
